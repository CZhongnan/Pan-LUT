...
from ._ext import (
    PGLUT_cforward, PGLUT_cbackward
)


class PGLUTTransformFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        PGLUT_cbackward(grad_output, img, lut, grad_img, grad_lut)
...
def PGLUT_transform(
    img: torch.Tensor,
    lut: torch.Tensor) -> torch.Tensor:
    r"""Spatil and Temporal 4D Lookup Table Transform (PGLUT-Transform).
    ...
    """
    return PGLUTTransformFunction.apply(img, lut)