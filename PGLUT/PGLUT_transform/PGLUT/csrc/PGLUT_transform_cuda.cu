...
void pglut_transform_sanity_check(
...
__global__ void pglut_transform_4d_cuda_forward_kernel(
        const int n,
    ...
    const int stride_lut_5 = stride_lut_4 * stride_lut;
    const int n_2=n*2;
    const int n_3=n*3;
    const int n_4=n*4;
    CUDA_1D_KERNEL_LOOP(index, n) {
        ...
        const scalar_t g = data_inp[index +  n];
        const scalar_t b = data_inp[index +  n_2];
        const scalar_t c = data_inp[index +  n_3];
        const scalar_t p = data_inp[index +  n_4];
        ...
        for (int i = 0; i < num_channels; ++i) {
            ...
            data_col[index +  height * width * i] =
                w00000 * data_lut[id00000 + stride_lut_5 * i] + w10000 * data_lut[id10000 + stride_lut_5 * i] +
                w01000 * data_lut[id01000 + stride_lut_5 * i] + w11000 * data_lut[id11000 + stride_lut_5 * i] +
                w00100 * data_lut[id00100 + stride_lut_5 * i] + w10100 * data_lut[id10100 + stride_lut_5 * i] +
                w01100 * data_lut[id01100 + stride_lut_5 * i] + w11100 * data_lut[id11100 + stride_lut_5 * i] +
                w00010 * data_lut[id00010 + stride_lut_5 * i] + w10010 * data_lut[id10010 + stride_lut_5 * i] +
                w01010 * data_lut[id01010 + stride_lut_5 * i] + w11010 * data_lut[id11010 + stride_lut_5 * i] +
                w00110 * data_lut[id00110 + stride_lut_5 * i] + w10110 * data_lut[id10110 + stride_lut_5 * i] +
                w01110 * data_lut[id01110 + stride_lut_5 * i] + w11110 * data_lut[id11110 + stride_lut_5 * i] +
                w00001 * data_lut[id00001 + stride_lut_5 * i] + w10001 * data_lut[id10001 + stride_lut_5 * i] +
                w01001 * data_lut[id01001 + stride_lut_5 * i] + w11001 * data_lut[id11001 + stride_lut_5 * i] +
                w00101 * data_lut[id00101 + stride_lut_5 * i] + w10101 * data_lut[id10101 + stride_lut_5 * i] +
                w01101 * data_lut[id01101 + stride_lut_5 * i] + w11101 * data_lut[id11101 + stride_lut_5 * i] +
                w00011 * data_lut[id00011 + stride_lut_5 * i] + w10011 * data_lut[id10011 + stride_lut_5 * i] +
                w01011 * data_lut[id01011 + stride_lut_5 * i] + w11011 * data_lut[id11011 + stride_lut_5 * i] +
                w00111 * data_lut[id00111 + stride_lut_5 * i] + w10111 * data_lut[id10111 + stride_lut_5 * i] +
                w01111 * data_lut[id01111 + stride_lut_5 * i] + w11111 * data_lut[id11111 + stride_lut_5 * i];
        }
    }
}
...
template <typename scalar_t>
__launch_bounds__(THREADS_PER_BLOCK)
__global__ void pglut_transform_4d_cuda_backward_kernel(
        const int n,
    ...
    CUDA_1D_KERNEL_LOOP(index, n) {
        ...
        for (int i = 0; i < num_channels; ++i) {
            ...
            atomicAdd(grad_lut + id00000 + stride_lut_5 * i, grad_o_ * w00000);
            atomicAdd(grad_lut + id10000 + stride_lut_5 * i, grad_o_ * w10000);
            atomicAdd(grad_lut + id01000 + stride_lut_5 * i, grad_o_ * w01000);
            atomicAdd(grad_lut + id11000 + stride_lut_5 * i, grad_o_ * w11000);
            atomicAdd(grad_lut + id00100 + stride_lut_5 * i, grad_o_ * w00100);
            atomicAdd(grad_lut + id10100 + stride_lut_5 * i, grad_o_ * w10100);
            atomicAdd(grad_lut + id01100 + stride_lut_5 * i, grad_o_ * w01100);
            atomicAdd(grad_lut + id11100 + stride_lut_5 * i, grad_o_ * w11100);
            atomicAdd(grad_lut + id00010 + stride_lut_5 * i, grad_o_ * w00010);
            atomicAdd(grad_lut + id10010 + stride_lut_5 * i, grad_o_ * w10010);
            atomicAdd(grad_lut + id01010 + stride_lut_5 * i, grad_o_ * w01010);
            atomicAdd(grad_lut + id11010 + stride_lut_5 * i, grad_o_ * w11010);
            atomicAdd(grad_lut + id00110 + stride_lut_5 * i, grad_o_ * w00110);
            atomicAdd(grad_lut + id10110 + stride_lut_5 * i, grad_o_ * w10110);
            atomicAdd(grad_lut + id01110 + stride_lut_5 * i, grad_o_ * w01110);
            atomicAdd(grad_lut + id11110 + stride_lut_5 * i, grad_o_ * w11110);
            atomicAdd(grad_lut + id00001 + stride_lut_5 * i, grad_o_ * w00001);
            atomicAdd(grad_lut + id10001 + stride_lut_5 * i, grad_o_ * w10001);
            atomicAdd(grad_lut + id01001 + stride_lut_5 * i, grad_o_ * w01001);
            atomicAdd(grad_lut + id11001 + stride_lut_5 * i, grad_o_ * w11001);
            atomicAdd(grad_lut + id00101 + stride_lut_5 * i, grad_o_ * w00101);
            atomicAdd(grad_lut + id10101 + stride_lut_5 * i, grad_o_ * w10101);
            atomicAdd(grad_lut + id01101 + stride_lut_5 * i, grad_o_ * w01101);
            atomicAdd(grad_lut + id11101 + stride_lut_5 * i, grad_o_ * w11101);
            atomicAdd(grad_lut + id00011 + stride_lut_5 * i, grad_o_ * w00011);
            atomicAdd(grad_lut + id10011 + stride_lut_5 * i, grad_o_ * w10011);
            atomicAdd(grad_lut + id01011 + stride_lut_5 * i, grad_o_ * w01011);
            atomicAdd(grad_lut + id11011 + stride_lut_5 * i, grad_o_ * w11011);
            atomicAdd(grad_lut + id00111 + stride_lut_5 * i, grad_o_ * w00111);
            atomicAdd(grad_lut + id10111 + stride_lut_5 * i, grad_o_ * w10111);
            atomicAdd(grad_lut + id01111 + stride_lut_5 * i, grad_o_ * w01111);
            atomicAdd(grad_lut + id11111 + stride_lut_5 * i, grad_o_ * w11111);

            /* compute gradient of vertices */
            scalar_t grad_d = 0;
            const scalar_t lut00000 = data_lut[id00000 + stride_lut_5 * i];
            const scalar_t lut10000 = data_lut[id10000 + stride_lut_5 * i];
            const scalar_t lut01000 = data_lut[id01000 + stride_lut_5 * i];
            const scalar_t lut11000 = data_lut[id11000 + stride_lut_5 * i];
            const scalar_t lut00100 = data_lut[id00100 + stride_lut_5 * i];
            const scalar_t lut10100 = data_lut[id10100 + stride_lut_5 * i];
            const scalar_t lut01100 = data_lut[id01100 + stride_lut_5 * i];
            const scalar_t lut11100 = data_lut[id11100 + stride_lut_5 * i];
            const scalar_t lut00010 = data_lut[id00010 + stride_lut_5 * i];
            const scalar_t lut10010 = data_lut[id10010 + stride_lut_5 * i];
            const scalar_t lut01010 = data_lut[id01010 + stride_lut_5 * i];
            const scalar_t lut11010 = data_lut[id11010 + stride_lut_5 * i];
            const scalar_t lut00110 = data_lut[id00110 + stride_lut_5 * i];
            const scalar_t lut10110 = data_lut[id10110 + stride_lut_5 * i];
            const scalar_t lut01110 = data_lut[id01110 + stride_lut_5 * i];
            const scalar_t lut11110 = data_lut[id11110 + stride_lut_5 * i];
            const scalar_t lut00001 = data_lut[id00001 + stride_lut_5 * i];
            const scalar_t lut10001 = data_lut[id10001 + stride_lut_5 * i];
            const scalar_t lut01001 = data_lut[id01001 + stride_lut_5 * i];
            const scalar_t lut11001 = data_lut[id11001 + stride_lut_5 * i];
            const scalar_t lut00101 = data_lut[id00101 + stride_lut_5 * i];
            const scalar_t lut10101 = data_lut[id10101 + stride_lut_5 * i];
            const scalar_t lut01101 = data_lut[id01101 + stride_lut_5 * i];
            const scalar_t lut11101 = data_lut[id11101 + stride_lut_5 * i];
            const scalar_t lut00011 = data_lut[id00011 + stride_lut_5 * i];
            const scalar_t lut10011 = data_lut[id10011 + stride_lut_5 * i];
            const scalar_t lut01011 = data_lut[id01011 + stride_lut_5 * i];
            const scalar_t lut11011 = data_lut[id11011 + stride_lut_5 * i];
            const scalar_t lut00111 = data_lut[id00111 + stride_lut_5 * i];
            const scalar_t lut10111 = data_lut[id10111 + stride_lut_5 * i];
            const scalar_t lut01111 = data_lut[id01111 + stride_lut_5 * i];
            const scalar_t lut11111 = data_lut[id11111 + stride_lut_5 * i];
            ...
            // c
            atomicAdd(grad_inp + index +  height * width * 3, grad_d * 1 / size_bin);
            ...
            // p
            atomicAdd(grad_inp + index +  height * width * 4, grad_d * 1 / size_bin);
        }
    }
}
...
void PGLUTTransformForwardCUDAKernelLauncher(
    ...
    // pglut_transform_sanity_check(input, lut, output);
    ...
    for (int elt = 0; elt < batch_size; ++elt) {
        ...
        AT_DISPATCH_FLOATING_TYPES(
            input.scalar_type(), "pglut_transform_cuda_forward", ([&] {
                ...
                pglut_transform_4d_cuda_forward_kernel<<<GET_BLOCKS(num_kernels),
                                                    THREADS_PER_BLOCK, 0,
...
void PGLUTTransformBackwardCUDAKernelLauncher(
    ...
    for (int elt = 0; elt < batch_size; ++elt) {
        ...
        /* launch the CUDA kernel */
        AT_DISPATCH_FLOATING_TYPES(
            input.scalar_type(), "pglut_transform_cuda_backward", ([&] {
                ...
                pglut_transform_4d_cuda_backward_kernel<<<GET_BLOCKS(num_kernels),
                                                    THREADS_PER_BLOCK, 0,
                                                    at::cuda::getCurrentCUDAStream()>>>(
