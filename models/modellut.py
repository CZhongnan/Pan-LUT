...
from AALUT import AALUT_transform
...
from AGLUT import AGLUT_transform
from PGLUTCPU import PGLUTCPU_transform
dimPG=9
dimSD=9
dimAA=9
...
class Generator5DLUT_identity(nn.Module):
                ...
                for k in range(0,dim):
                    for l in range(0,dim):
                        for m in range(0,dim):
                            ...
                            buffer[0,3,i,j,k,l,m] = float(x[3])
        self.LUT = nn.Parameter(torch.from_numpy(buffer).requires_grad_(True))
        # self.FivetilinearInterpolation = FivetilinearInterpolation()
    ...
    def regularizations(self, smoothness, monotonicity):
        basis_luts = self.LUT
        tv, mn = 0, 0
        for i in range(1, basis_luts.ndimension()):
...
class Generator3DLUTL5_identity(nn.Module):
    def __init__(self, dim=dimSD,num_batch=5):
        ...
        # self.TrilinearLInterpolation = TrilinearLInterpolation()

    def forward(self, x):
        # _, output = self.TrilinearLInterpolation(self.LUT, x)
...
class Generator3DLUTL270_identity(nn.Module):
    ...
    def forward(self, x):
        # _, output = self.TrilinearL270Interpolation(self.LUT, x)
