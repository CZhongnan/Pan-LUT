...
void if4dlut_transform_sanity_check(
...
__global__ void if4dlut_transform_4d_cuda_forward_kernel(
        const int n,
...
template <typename scalar_t>
__launch_bounds__(THREADS_PER_BLOCK)
__global__ void if4dlut_transform_4d_cuda_backward_kernel(
        const int n,
...
void IF4DLUTTransformForwardCUDAKernelLauncher(
    ...
    // if4dlut_transform_sanity_check(input, lut, output);
    ...
    for (int elt = 0; elt < batch_size; ++elt) {
        ...
        AT_DISPATCH_FLOATING_TYPES(
            input.scalar_type(), "if4dlut_transform_cuda_forward", ([&] {
                ...
                if4dlut_transform_4d_cuda_forward_kernel<<<GET_BLOCKS(num_kernels),
                                                    THREADS_PER_BLOCK, 0,
...
void IF4DLUTTransformBackwardCUDAKernelLauncher(
    ...
    for (int elt = 0; elt < batch_size; ++elt) {
        ...
        /* launch the CUDA kernel */
        AT_DISPATCH_FLOATING_TYPES(
            input.scalar_type(), "if4dlut_transform_cuda_backward", ([&] {
                ...
                if4dlut_transform_4d_cuda_backward_kernel<<<GET_BLOCKS(num_kernels),
                                                    THREADS_PER_BLOCK, 0,
                                                    at::cuda::getCurrentCUDAStream()>>>(
