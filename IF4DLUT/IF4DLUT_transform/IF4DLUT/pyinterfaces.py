...
from ._ext import (
    IF4DLUT_cforward, IF4DLUT_cbackward
)


class IF4DLUTTransformFunction(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        IF4DLUT_cforward(img, lut, output)
    ...
    def backward(ctx, grad_output: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        IF4DLUT_cbackward(grad_output, img, lut, grad_img, grad_lut)

        return grad_img, grad_lut


def IF4DLUT_transform(
    img: torch.Tensor,
    lut: torch.Tensor) -> torch.Tensor:
    r"""Spatil and Temporal 4D Lookup Table Transform (IF4DLUT-Transform).
    ...
    """
    return IF4DLUTTransformFunction.apply(img, lut)