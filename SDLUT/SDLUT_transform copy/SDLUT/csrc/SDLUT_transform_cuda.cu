...
__global__ void sdlut_transform_4d_cuda_forward_kernel(
    ...
    CUDA_1D_KERNEL_LOOP(index, n) {
        ...
        const scalar_t g =  data_inp[index + 
            (y < 1 ? width : -width) + 
            (x == 0 ? 1 : -1)];
        const scalar_t b = data_inp[index + (y < 1 ? 0 : -width)];
        const scalar_t c = data_inp[index + 
            (y < 1 ? width : -width) + 
            (x >= width - 1 ? -1 : 1)];
        const scalar_t b1 = data_inp[index + (x == 0 ? 0 : -1)];
        const scalar_t b2 = data_inp[index + (x >= width - 1 ? 0 : 1)];
        const scalar_t b3 = data_inp[index + 
            (y >= height - 1 ? -width : width) + 
            (x == 0 ? 1 : -1)];
        const scalar_t b4 = data_inp[index + (y >= height - 1 ? -width : width)];
        const scalar_t b5 = data_inp[index + 
            (y >= height - 1 ? -width : width) + 
            (x >= width - 1 ? -1 : 1)];
        // const scalar_t b = data_inp[index + (y < 1 ? 0 : -width)];
        // const scalar_t r = data_inp[index];
        ...
        const int32_t b1id = clamp((int32_t)floor(b1 * (stride_lut - 1)), 0, stride_lut - 2);
        const int32_t b2id = clamp((int32_t)floor(b2 * (stride_lut - 1)), 0, stride_lut - 2);
        const int32_t b3id = clamp((int32_t)floor(b3 * (stride_lut - 1)), 0, stride_lut - 2);
        const int32_t b4id = clamp((int32_t)floor(b4 * (stride_lut - 1)), 0, stride_lut - 2);
        const int32_t b5id = clamp((int32_t)floor(b5 * (stride_lut - 1)), 0, stride_lut - 2);
        ...
        const int stride_lut_6 = stride_lut_5 * stride_lut;
        const int stride_lut_7 = stride_lut_6 * stride_lut;
        const int stride_lut_8 = stride_lut_7 * stride_lut;
        ...
        /* retrieve the interpolation verticess for 9D (rid, gid, bid, cid, b1id, b2id, b3id, b4id, b5id) */
        const int id000000000 = (rid    ) + stride_lut * (gid    ) + stride_lut_2 * (bid    ) + stride_lut_3 * (cid    ) + stride_lut_4 * (b1id    ) + stride_lut_5 * (b2id    ) + stride_lut_6 * (b3id    ) + stride_lut_7 * (b4id    ) + stride_lut_8 * (b5id    );
        const int id100000000 = (rid + 1) + stride_lut * (gid    ) + stride_lut_2 * (bid    ) + stride_lut_3 * (cid    ) + stride_lut_4 * (b1id    ) + stride_lut_5 * (b2id    ) + stride_lut_6 * (b3id    ) + stride_lut_7 * (b4id    ) + stride_lut_8 * (b5id    );
        const int id010000000 = (rid    ) + stride_lut * (gid + 1) + stride_lut_2 * (bid    ) + stride_lut_3 * (cid    ) + stride_lut_4 * (b1id    ) + stride_lut_5 * (b2id    ) + stride_lut_6 * (b3id    ) + stride_lut_7 * (b4id    ) + stride_lut_8 * (b5id    );
        const int id001000000 = (rid    ) + stride_lut * (gid    ) + stride_lut_2 * (bid + 1) + stride_lut_3 * (cid    ) + stride_lut_4 * (b1id    ) + stride_lut_5 * (b2id    ) + stride_lut_6 * (b3id    ) + stride_lut_7 * (b4id    ) + stride_lut_8 * (b5id    );
        const int id000100000 = (rid    ) + stride_lut * (gid    ) + stride_lut_2 * (bid    ) + stride_lut_3 * (cid + 1) + stride_lut_4 * (b1id    ) + stride_lut_5 * (b2id    ) + stride_lut_6 * (b3id    ) + stride_lut_7 * (b4id    ) + stride_lut_8 * (b5id    );
        const int id000010000 = (rid    ) + stride_lut * (gid    ) + stride_lut_2 * (bid    ) + stride_lut_3 * (cid    ) + stride_lut_4 * (b1id + 1) + stride_lut_5 * (b2id    ) + stride_lut_6 * (b3id    ) + stride_lut_7 * (b4id    ) + stride_lut_8 * (b5id    );
        const int id000001000 = (rid    ) + stride_lut * (gid    ) + stride_lut_2 * (bid    ) + stride_lut_3 * (cid    ) + stride_lut_4 * (b1id    ) + stride_lut_5 * (b2id + 1) + stride_lut_6 * (b3id    ) + stride_lut_7 * (b4id    ) + stride_lut_8 * (b5id    );
        const int id000000100 = (rid    ) + stride_lut * (gid    ) + stride_lut_2 * (bid    ) + stride_lut_3 * (cid    ) + stride_lut_4 * (b1id    ) + stride_lut_5 * (b2id    ) + stride_lut_6 * (b3id + 1) + stride_lut_7 * (b4id    ) + stride_lut_8 * (b5id    );
        const int id000000010 = (rid    ) + stride_lut * (gid    ) + stride_lut_2 * (bid    ) + stride_lut_3 * (cid    ) + stride_lut_4 * (b1id    ) + stride_lut_5 * (b2id    ) + stride_lut_6 * (b3id    ) + stride_lut_7 * (b4id + 1) + stride_lut_8 * (b5id    );
        const int id000000001 = (rid    ) + stride_lut * (gid    ) + stride_lut_2 * (bid    ) + stride_lut_3 * (cid    ) + stride_lut_4 * (b1id    ) + stride_lut_5 * (b2id    ) + stride_lut_6 * (b3id    ) + stride_lut_7 * (b4id    ) + stride_lut_8 * (b5id + 1);
        const int id111111111 = (rid + 1) + stride_lut * (gid + 1) + stride_lut_2 * (bid + 1) + stride_lut_3 * (cid + 1) + stride_lut_4 * (b1id + 1) + stride_lut_5 * (b2id + 1) + stride_lut_6 * (b3id + 1) + stride_lut_7 * (b4id + 1) + stride_lut_8 * (b5id + 1);
...
__global__ void sdlut_transform_4d_cuda_backward_kernel(
    ...
    CUDA_1D_KERNEL_LOOP(index, n) {
        ...
        /* retrieve rgbc value of the pixel */
        const scalar_t r = data_inp[index];
        const scalar_t g = data_inp[index + (x >= width - 1 ? -1 : 1)];
        const scalar_t b = data_inp[index + (y >= height - 1 ? -width : width)];
        const scalar_t c = data_inp[index + 
            (y >= height - 1 ? -width : width) + 
            (x >= width - 1 ? -1 : 1)];
        /* retrieve index of the interpolation verticess */
        ...
        for (int i = 0; i < num_channels; ++i) {
            ...
            // g
            atomicAdd(grad_inp + index + (x >= width - 1 ? -1 : 1), grad_d * 1 / size_bin);
            ...
            // b
            atomicAdd(grad_inp + index + (y >= height - 1 ? -width : width), grad_d * 1 / size_bin);
    ...
    (y >= height - 1 ? -width : width) + 
    (x >= width - 1 ? -1 : 1), grad_d * 1 / size_bin);
        }
    }
}
