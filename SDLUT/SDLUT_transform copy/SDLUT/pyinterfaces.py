...
from ._ext import (
    SDLUT_cforward, SDLUT_cbackward
)


class SDLUTTransformFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        SDLUT_cbackward(grad_output, img, lut, grad_img, grad_lut)
...
def SDLUT_transform(
    img: torch.Tensor,
    lut: torch.Tensor) -> torch.Tensor:
    r"""Spatil and Temporal 4D Lookup Table Transform (SDLUT-Transform).
    ...
    """
    return SDLUTTransformFunction.apply(img, lut)