...
from ._ext import (
    SD270LUT_cforward, SD270LUT_cbackward
)


class SD270LUTTransformFunction(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        SD270LUT_cforward(img, lut, output)
    ...
    def backward(ctx, grad_output: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        SD270LUT_cbackward(grad_output, img, lut, grad_img, grad_lut)

        return grad_img, grad_lut


def SD270LUT_transform(
    img: torch.Tensor,
    lut: torch.Tensor) -> torch.Tensor:
    r"""Spatil and Temporal 4D Lookup Table Transform (SD270LUT-Transform).
    ...
    """
    return SD270LUTTransformFunction.apply(img, lut)