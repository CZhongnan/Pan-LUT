...
__global__ void sd270lut_transform_4d_cuda_forward_kernel(
        const int n,
    ...
    CUDA_1D_KERNEL_LOOP(index, n) {
        ...
        const scalar_t g = data_inp[index + (x >= width - 1 ? 0 : 1)];
...
template <typename scalar_t>
__launch_bounds__(THREADS_PER_BLOCK)
__global__ void sd270lut_transform_4d_cuda_backward_kernel(
        const int n,
    ...
    (y < 1 ? 0 : -width) + 
    (x >= width - 1 ? 0 : 1), grad_d * 1 / size_bin);
        }
}
...
void SD270LUTTransformForwardCUDAKernelLauncher(
    ...
    // sd270lut_transform_sanity_check(input, lut, output);
    ...
    for (int elt = 0; elt < num_channels; ++elt) {
        ...
        AT_DISPATCH_FLOATING_TYPES(
            input.scalar_type(), "sd270lut_transform_cuda_forward", ([&] {
                ...
                sd270lut_transform_4d_cuda_forward_kernel<<<GET_BLOCKS(num_kernels),
                                                    THREADS_PER_BLOCK, 0,
...
void SD270LUTTransformBackwardCUDAKernelLauncher(
    ...
    for (int elt = 0; elt < num_channels; ++elt) {
        ...
        /* launch the CUDA kernel */
        AT_DISPATCH_FLOATING_TYPES(
            input.scalar_type(), "sd270lut_transform_cuda_backward", ([&] {
                ...
                sd270lut_transform_4d_cuda_backward_kernel<<<GET_BLOCKS(num_kernels),
                                                    THREADS_PER_BLOCK, 0,
                                                    at::cuda::getCurrentCUDAStream()>>>(
