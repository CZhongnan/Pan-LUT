...
void sd180lut_transform_sanity_check(
...
__global__ void sd180lut_transform_4d_cuda_forward_kernel(
        const int n,
    ...
    CUDA_1D_KERNEL_LOOP(index, n) {
        ...
        const scalar_t g = data_inp[index + (x == 0 ? 0 : -1)];
...
template <typename scalar_t>
__launch_bounds__(THREADS_PER_BLOCK)
__global__ void sd180lut_transform_4d_cuda_backward_kernel(
        const int n,
    ...
    CUDA_1D_KERNEL_LOOP(index, n) {
        ...
        const scalar_t w1111_cd =   (    rd) * (    gd) * (    bd);
            scalar_t grad_o_ = grad_output[index];
...
void SD180LUTTransformForwardCUDAKernelLauncher(
    ...
    // sd180lut_transform_sanity_check(input, lut, output);
    ...
    for (int elt = 0; elt < num_channels; ++elt) {
        ...
        AT_DISPATCH_FLOATING_TYPES(
            input.scalar_type(), "sd180lut_transform_cuda_forward", ([&] {
                const scalar_t *data_inp = input[0][elt].data_ptr<scalar_t>();
                const scalar_t *data_lut = lut[elt].data_ptr<scalar_t>();
                scalar_t *data_col = output[0][elt].data_ptr<scalar_t>();

                sd180lut_transform_4d_cuda_forward_kernel<<<GET_BLOCKS(num_kernels),
                                                    THREADS_PER_BLOCK, 0,
...
void SD180LUTTransformBackwardCUDAKernelLauncher(
    ...
    for (int elt = 0; elt < num_channels; ++elt) {
        ...
        /* launch the CUDA kernel */
        AT_DISPATCH_FLOATING_TYPES(
            input.scalar_type(), "sd180lut_transform_cuda_backward", ([&] {
                const scalar_t *grad_out = grad_output[0][elt].data_ptr<scalar_t>();
                const scalar_t *data_inp = input[0][elt].data_ptr<scalar_t>();
                const scalar_t *data_lut = lut[elt].data_ptr<scalar_t>();
                scalar_t *grad_inp_  = grad_inp[0][elt].data_ptr<scalar_t>();
                scalar_t *grad_lut_ = grad_lut[elt].data_ptr<scalar_t>();

                sd180lut_transform_4d_cuda_backward_kernel<<<GET_BLOCKS(num_kernels),
                                                    THREADS_PER_BLOCK, 0,
                                                    at::cuda::getCurrentCUDAStream()>>>(
