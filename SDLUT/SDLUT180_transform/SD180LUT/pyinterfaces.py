...
from ._ext import (
    SD180LUT_cforward, SD180LUT_cbackward
)


class SD180LUTTransformFunction(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        SD180LUT_cforward(img, lut, output)
    ...
    def backward(ctx, grad_output: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        SD180LUT_cbackward(grad_output, img, lut, grad_img, grad_lut)

        return grad_img, grad_lut


def SD180LUT_transform(
    img: torch.Tensor,
    lut: torch.Tensor) -> torch.Tensor:
    r"""Spatil and Temporal 4D Lookup Table Transform (SD180LUT-Transform).
    ...
    """
    return SD180LUTTransformFunction.apply(img, lut)