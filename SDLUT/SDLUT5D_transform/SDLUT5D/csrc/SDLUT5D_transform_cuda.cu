...
void sdlut5D_transform_sanity_check(
...
__global__ void sdlut5D_transform_4d_cuda_forward_kernel(
        const int n,
...
template <typename scalar_t>
__launch_bounds__(THREADS_PER_BLOCK)
__global__ void sdlut5D_transform_4d_cuda_backward_kernel(
        const int n,
    ...
    CUDA_1D_KERNEL_LOOP(index, n) {
        ...
        for (int i = 0; i < num_channels; ++i) {
            ...
            // r
            atomicAdd(grad_inp + index+ (y < 1 ? 0 : -width) + 
            ...
            // b
            atomicAdd(grad_inp + index + (y >= height - 1 ? 0 : width) + 
            (x == 0 ? 0 : -1), grad_d * 1 / size_bin);
...
void SDLUT5DTransformForwardCUDAKernelLauncher(
    ...
    // sdlut5D_transform_sanity_check(input, lut, output);
    ...
    for (int elt = 0; elt < batch_size; ++elt) {
        ...
        AT_DISPATCH_FLOATING_TYPES(
            input.scalar_type(), "sdlut5D_transform_cuda_forward", ([&] {
                ...
                sdlut5D_transform_4d_cuda_forward_kernel<<<GET_BLOCKS(num_kernels),
                                                    THREADS_PER_BLOCK, 0,
...
void SDLUT5DTransformBackwardCUDAKernelLauncher(
    ...
    for (int elt = 0; elt < batch_size; ++elt) {
        ...
        /* launch the CUDA kernel */
        AT_DISPATCH_FLOATING_TYPES(
            input.scalar_type(), "sdlut5D_transform_cuda_backward", ([&] {
                ...
                sdlut5D_transform_4d_cuda_backward_kernel<<<GET_BLOCKS(num_kernels),
                                                    THREADS_PER_BLOCK, 0,
                                                    at::cuda::getCurrentCUDAStream()>>>(
