...
from ._ext import (
    SDLUT5D_cforward, SDLUT5D_cbackward
)


class SDLUT5DTransformFunction(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        SDLUT5D_cforward(img, lut, output)
    ...
    def backward(ctx, grad_output: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        SDLUT5D_cbackward(grad_output, img, lut, grad_img, grad_lut)

        return grad_img, grad_lut


def SDLUT5D_transform(
    img: torch.Tensor,
    lut: torch.Tensor) -> torch.Tensor:
    r"""Spatil and Temporal 4D Lookup Table Transform (SDLUT5D-Transform).
    ...
    """
    return SDLUT5DTransformFunction.apply(img, lut)