...
void sd90lut_transform_sanity_check(
...
__global__ void sd90lut_transform_4d_cuda_forward_kernel(
        const int n,
    ...
    CUDA_1D_KERNEL_LOOP(index, n) {
        ...
        const scalar_t b = data_inp[index + (y >= height - 1 ? 0 : width)];
...
template <typename scalar_t>
__launch_bounds__(THREADS_PER_BLOCK)
__global__ void sd90lut_transform_4d_cuda_backward_kernel(
        const int n,
...
void SD90LUTTransformForwardCUDAKernelLauncher(
    ...
    // sd90lut_transform_sanity_check(input, lut, output);
    ...
    for (int elt = 0; elt < num_channels; ++elt) {
        ...
        AT_DISPATCH_FLOATING_TYPES(
            input.scalar_type(), "sd90lut_transform_cuda_forward", ([&] {
                ...
                sd90lut_transform_4d_cuda_forward_kernel<<<GET_BLOCKS(num_kernels),
                                                    THREADS_PER_BLOCK, 0,
...
void SD90LUTTransformBackwardCUDAKernelLauncher(
    ...
    for (int elt = 0; elt < num_channels; ++elt) {
        ...
        /* launch the CUDA kernel */
        AT_DISPATCH_FLOATING_TYPES(
            input.scalar_type(), "sd90lut_transform_cuda_backward", ([&] {
                ...
                sd90lut_transform_4d_cuda_backward_kernel<<<GET_BLOCKS(num_kernels),
                                                    THREADS_PER_BLOCK, 0,
                                                    at::cuda::getCurrentCUDAStream()>>>(
