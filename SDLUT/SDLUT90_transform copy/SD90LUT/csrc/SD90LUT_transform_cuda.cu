...
__global__ void sd90lut_transform_4d_cuda_forward_kernel(
    ...
    CUDA_1D_KERNEL_LOOP(index, n) {
        ...
        const scalar_t g = data_inp[index + (y >= height - 1 ? -width : width)];
        const scalar_t b = data_inp[index + (y < 1 ? width : -width)];
...
__global__ void sd90lut_transform_4d_cuda_backward_kernel(
    ...
    CUDA_1D_KERNEL_LOOP(index, n) {
        ...
        const scalar_t r = data_inp[index];
        const scalar_t g = data_inp[index + (y >= height - 1 ? -width : width)];
        const scalar_t b = data_inp[index + (x == 0 ? 1 : -1)];
        const scalar_t c = data_inp[index + 
            (y >= height - 1 ? -width : width) + 
            (x == 0 ? 1 : -1)];
        /* retrieve index of the interpolation verticess */
        ...
        for (int i = 0; i < num_channels; ++i) {
            ...
            // b
            atomicAdd(grad_inp + index + (x == 0 ? 1 : -1), grad_d * 1 / size_bin);
